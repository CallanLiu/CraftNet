...
public class MessageSystem : IMessageSystem
{
    ...
    public MessageSystem(App app)
    {
        ...
        if (app.IsFirstLoad)
        {
            _state = app.AddComponent<MessageSystemState>();
        }

        _state = app.GetComponent<MessageSystemState>();
    ...
    {
        _state.Handlers.Add(T.Opcode, ActivatorUtilities.CreateInstance<T>(_app.Services));
    }
    ...
    public void RegisterFilter<T>() where T : IMessageFilter, new()
    {
        ...
        // 如果不存在就创建
        _state.MessageFilters[T.Id] ??= ActivatorUtilities.CreateInstance<T>(_app.Services);
    }
